...
class RoleController extends Controller
{
    ...
    {
        $user = $request->user();
        if ($user && !$user->can('Roles & Akses Create') && !$user->can('Roles Create') && !$user->can('create roles') && !$user->can('manage all')) {
    ...
    {
        $user = $request->user();
        if ($user && !$user->can('Roles & Akses Write') && !$user->can('Roles Write') && !$user->can('write roles') && !$user->can('manage all')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    {
        $user = $request->user();
        if ($user && !$user->can('Roles & Akses Delete') && !$user->can('Roles Delete') && !$user->can('delete roles') && !$user->can('manage all')) {
            abort(403, 'Unauthorized action.');
        }
